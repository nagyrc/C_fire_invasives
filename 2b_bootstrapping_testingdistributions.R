...
ABGCsb <- subset.data.frame(rawdata, pool == "AGBC_g_m2" &
                              veg == "sagebrush")
...
ABGCsb <- ABGCsb %>%
  ...
  mutate(SE = sqrt(var(pool_value, na.rm=TRUE)/
                     sum(!is.na(pool_value)))) %>%
...
ObsDF = data.frame(Type="Observed", 
                   pool_value=ABGCsb$pool_value)
...
DistGamma2DF = data.frame(Type = "Gamma2",
                     pool_value = rgamma(n= N, 
                                         shape= shape, 
                                         scale= scale))
...
OGSCcg <- subset.data.frame(rawdata, pool == "orgsoilC_g_m2" &
                              veg == "cheatgrass")
...
## Bootstrap sampling
Boot1 = data.frame(Type="Boot1", 
                     pool_value = sample(OGSCcg$pool_value,
                                         size=N, replace=TRUE))
